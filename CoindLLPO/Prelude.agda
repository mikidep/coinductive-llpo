module CoindLLPO.Prelude where

open import Cubical.Foundations.Prelude public
open import Cubical.Foundations.Function using (_∘_; flip; ∘-assoc)
open import Cubical.Functions.FunExtEquiv using (funExtDep) public
...
module ΣDepPathChar {ℓ ℓ'} {A : Type ℓ} {B : A → Type ℓ'} where
  Σ≡equiv : ∀ (x y : Σ A B)
    → (Σ[ p ∈ (fst x ≡ fst y) ] (snd x ≡[ p , B ] snd y)) ≃ (x ≡ y)
  Σ≡equiv x y = ΣPath≃PathΣ
